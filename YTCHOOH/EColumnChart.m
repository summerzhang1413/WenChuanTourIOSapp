...
#define Y_COORDINATE_LABEL_WIDTH 0
...
- (void)reloadData
{
    ...
    NSMutableArray *outOfRangeIndexArray = [[NSMutableArray alloc] initWithCapacity:33];
...
//            maxValue = eColumnDataModel.value;
            [outOfRangeIndexArray addObject:[NSString stringWithFormat:@"%d", currentIndex]];
        }
...
//        eColumn.barColor = _maxColumnColor;
        ...
        for (int i = 0; i < [outOfRangeIndexArray count]; i ++) {
            //            EColumn *eColumn = [_eColumns objectForKey: [NSNumber numberWithInteger: maxIndex]];
            EColumn *eColumn = [_eColumns objectForKey:[NSNumber numberWithInt:[outOfRangeIndexArray[i] intValue]]];
            eColumn.barColor = _maxColumnColor;
        }
        
        EColumn *eColumn = [_eColumns objectForKey: [NSNumber numberWithInteger: minIndex]];
